...
def calculate_average_difference(filename, total_index):
    total_sum = 0
    count = 0
        ...
        if txt_files:
            ...
            for txt_file in txt_files:
                ...
                with open(txt_path, "r") as file:
                    ...
                    total_index = title.index("total") if "total" in title else -1
                    if total_index != -1:
                        average_difference = calculate_average_difference(
                            txt_path, total_index
                        )
                        subfolder_average += average_difference
                        subfolder_files += 1
                    else:
                        print(f"Warning: 'total' column not found in {txt_path}")

            if subfolder_files > 0:
                print(
                    f"Average Total-Tracking for subfolder {root}: {subfolder_average/subfolder_files:.2f}"
                )
                total_average += subfolder_average

    if total_files > 0:
        overall_average = total_average / total_files
        print(f"Overall Average Total-Tracking for all files: {overall_average:.2f}")
    else:
