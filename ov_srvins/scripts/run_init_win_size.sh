...
setup_ls="num_kf:=$num_kf,do_bg:=false,init_feat_num:=200,bag_durr:=10,max_cameras:=1,init_num_iter:=0,do_calibration:=false,win_time:=${win_time[a]}"
...
sys_name1="nc_0iter_${win_time[a]}"
sys_name2="nc_10iter_${win_time[a]}"

bash ${run_script} ${dataset[e]} $sys_name1 "$HOME/results/srf_init/$run_name" "0" "${setup_ls}"
...
run_name="win_size_${dataset[$e]}_stereo"
...
setup_ls="num_kf:=$num_kf,do_bg:=false,init_feat_num:=200,bag_durr:=10,max_cameras:=2,init_num_iter:=0,do_calibration:=false,win_time:=${win_time[a]}"
