...
bag_path="/home/$username/datasets/$dataset"
...
for i in ${!sequences[@]}; do
  ...
  max_start_t=`python -c "f=open(\"$gt_file\"); ts=[float(line.split()[0]) for line in f.readlines() if line[0]!='#']; print($window_length*int((ts[-1]-ts[0])/$window_length-1)); f.close()"`
...
#         cpu_usage=$(top -b -n1 | grep "Cpu(s)" | awk '{print $2 + $4}')
#         # Check if the CPU usage is greater than the maximum allowed
#         if (( $(echo "$cpu_usage < $idle_cpu_usage" | bc -l) )); then
#           break
#         fi
#     fi
#     sleep 0.1
...
if [ $do_eval -eq 1 ]; then
  ...
  DISPLAY=:0.0 && ${script_dir}/eval.sh $run_name "init" &> "$outdir/init/results_traj.txt"
  DISPLAY=:0.0 && ${script_dir}/eval.sh $run_name "vio" &> "$outdir/vio/results_traj.txt"
