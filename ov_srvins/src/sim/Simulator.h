...
#ifndef OV_SRVINS_SIMULATOR_H
#define OV_SRVINS_SIMULATOR_H
...
public:
   ...
   */
  bool get_next_imu(double &time_imu, Vec3 &wm, Vec3 &am);
  ...
  std::unordered_map<size_t, Vec3> get_map() { return featmap; }

  /// Returns the true 3d map of features
  std::vector<Vec3> get_map_vec() {
    std::vector<Vec3> feats;
    for (auto const &feat : featmap)
      feats.push_back(feat.second.cast<DataType>());
    return feats;
  }
...
protected:
  ...
  project_pointcloud(const Mat3 &R_GtoI, const Vec3 &p_IinG, int camid,
   ...
   */
  void generate_points(const Mat3 &R_GtoI, const Vec3 &p_IinG, int camid,
                       std::unordered_map<size_t, Vec3> &feats, int numpts);
  ...
  size_t id_map = 0;
  std::unordered_map<size_t, Vec3> featmap;
  ...
  Vec3 true_bias_accel = Vec3::Zero();

  /// Our running gyroscope bias
  Vec3 true_bias_gyro = Vec3::Zero();

  // Our history of true biases
  bool has_skipped_first_bias = false;
  std::vector<double> hist_true_bias_time;
  std::vector<Vec3> hist_true_bias_accel;
  std::vector<Vec3> hist_true_bias_gyro;
};

} // namespace ov_srvins

#endif // OV_SRVINS_SIMULATOR_H
