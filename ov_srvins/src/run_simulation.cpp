...
int main(int argc, char **argv) {
  ...
  // Get initial state
  // NOTE: we are getting it at the *next* timestep so we get the first IMU
  // message
  ...
  if (!success) {
    PRINT_ERROR(
        RED
        "[SIM]: Could not initialize the filter to the first state\n" RESET);
  ...
  while (sim->ok()) {
    ...
    bool hasimu = sim->get_next_imu(message_imu.timestamp, message_imu.wm,
                                    message_imu.am);
    if (hasimu) {
...
#endif
        sys->feed_measurement_simulation(buffer_timecam, buffer_camids,
                                         buffer_feats);
#if ROS_AVAILABLE == 1 || ROS_AVAILABLE == 2
        viz->visualize();
#endif
