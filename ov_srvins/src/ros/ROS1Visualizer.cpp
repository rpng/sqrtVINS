...
ROS1Visualizer::ROS1Visualizer(std::shared_ptr<ros::NodeHandle> nh,
  ...
  pub_poseimu =
      nh->advertise<geometry_msgs::PoseWithCovarianceStamped>("poseimu", 2);
...
void ROS1Visualizer::setup_subscribers(
  ...
  sub_imu =
      _nh->subscribe(topic_imu, 1000, &ROS1Visualizer::callback_inertial, this);
  ...
  if (_app->get_params().state_options.num_cameras == 2) {
    ...
    _nh->param<std::string>("topic_camera" + std::to_string(0), cam_topic0,
                            "/cam" + std::to_string(0) + "/image_raw");
    _nh->param<std::string>("topic_camera" + std::to_string(1), cam_topic1,
    ...
    sync->registerCallback(
        boost::bind(&ROS1Visualizer::callback_stereo, this, _1, _2, 0, 1));
    ...
    for (int i = 0; i < _app->get_params().state_options.num_cameras; i++) {
      ...
      _nh->param<std::string>("topic_camera" + std::to_string(i), cam_topic,
      ...
      subs_cam.push_back(_nh->subscribe<sensor_msgs::Image>(
          cam_topic, 10,
          boost::bind(&ROS1Visualizer::callback_monocular, this, _1, i)));
...
void ROS1Visualizer::visualize_odometry(double timestamp) {
  ...
  tf::StampedTransform trans =
      ROSVisualizerHelper::get_stamped_transform_from_pose(odom_pose, false);
  ...
  for (const auto &calib : state->calib_IMUtoCAM) {
    tf::StampedTransform trans_calib =
        ROSVisualizerHelper::get_stamped_transform_from_pose(calib.second,
                                                             true);
...
void ROS1Visualizer::publish_features() {
  ...
  if (pub_points_msckf.getNumSubscribers() == 0 &&
      pub_points_slam.getNumSubscribers() == 0 &&
      pub_points_aruco.getNumSubscribers() == 0 &&
  ...
  sensor_msgs::PointCloud2 cloud =
      ROSVisualizerHelper::get_ros_pointcloud(feats_msckf);
  ...
  sensor_msgs::PointCloud2 cloud_SLAM =
      ROSVisualizerHelper::get_ros_pointcloud(feats_slam);
  ...
  sensor_msgs::PointCloud2 cloud_ARUCO =
      ROSVisualizerHelper::get_ros_pointcloud(feats_aruco);
  ...
  sensor_msgs::PointCloud2 cloud_SIM =
      ROSVisualizerHelper::get_ros_pointcloud(feats_sim);
...
void ROS1Visualizer::publish_groundtruth() {
  ...
  // We want to publish in the IMU clock frame
  // The timestamp in the state will be the last camera time
  DataType t_ItoC = _app->get_state()->calib_dt_CAMtoIMU->value()(0);
  ...
  tf::Quaternion quat(state_gt(1, 0), state_gt(2, 0), state_gt(3, 0),
                      state_gt(4, 0));
  ...
  std::vector<std::shared_ptr<Type>> qvars;
  std::vector<std::shared_ptr<Type>> pvars;
  qvars.push_back(_app->get_state()->imu->q());
  pvars.push_back(_app->get_state()->imu->p());
  ...
  //  Vec3 cov_vec = covariance.block(0, 0, 3, 3).inverse() * 2 *
  //  quat_diff.block(0, 0, 3, 1); DataType ori_nees = 2 *
  //  quat_diff_vec.dot(cov_vec); Vec3 errpos = state_ekf.block(4, 0, 3, 1) -
  //  state_gt.block(5, 0, 3, 1).cast<DataType>(); DataType pos_nees =
  //  errpos.transpose() * covariance.block(3, 3, 3, 3).inverse() * errpos;

  MatX Uq = StateHelper::get_marginal_U(_app->get_state(), qvars);
  MatX Up = StateHelper::get_marginal_U(_app->get_state(), pvars);
  efficient_QR(Uq);
  efficient_QR(Up);
  Uq.conservativeResizeLike(MatX::Zero(3, 3));
  Up.conservativeResizeLike(MatX::Zero(3, 3));

  Vec3 errori = 2 * quat_diff.block(0, 0, 3, 1);
  VecX Uq_eq = Uq.transpose().triangularView<Eigen::Lower>().solve(errori);
  DataType ori_nees = Uq_eq.transpose() * Uq_eq;

  Vec3 errpos =
      state_ekf.block(4, 0, 3, 1) - state_gt.block(5, 0, 3, 1).cast<DataType>();
  VecX Up_ep = Up.transpose().triangularView<Eigen::Lower>().solve(errpos);
  DataType pos_nees = Up_ep.transpose() * Up_ep;
