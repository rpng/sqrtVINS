...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < names.size(); i++) {
    ...
    PRINT_INFO(
        "mean_time = %.5f | std = %.5f | 99th = %.5f  | max = %.5f (%s)\n",
  ...
  double endtime1 =
      (times_skipped.empty()) ? 0 : times_skipped.at(times_skipped.size() - 1);
  ...
  // std::vector<std::string> colors_valid =
  // {"blue","aqua","lightblue","lightgreen","yellowgreen","green"};
  std::vector<std::string> colors_valid = {"navy",  "blue", "lightgreen",
                                           "green", "gold", "goldenrod"};
