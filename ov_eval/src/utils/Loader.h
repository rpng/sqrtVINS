...
public:
   ...
   * @param cov_ori Vector of orientation covariances at each timestep (empty if
   * we can't load)
   * @param cov_pos Vector of position covariances at each timestep (empty if we
   * can't load)
   */
  ...
  /**
   * @brief This will load *comma* separated trajectory into memory (ASL/ETH
   * format)
   * @param path_traj Path to the trajectory file that we want to read in.
   * @param times Timesteps in seconds for each pose
   * @param poses Pose at every timestep [pos,quat]
  ...
  /**
   * @brief Load an arbitrary sized row of *space* separated values, used for
   * our simulation
   * @param path Path to our text file to load
   * @param values Each row of values
   */
  static void load_simulation(std::string path,
   ...
   */
  static void
  load_timing_flamegraph(std::string path, std::vector<std::string> &names,
   ...
   */
  static double
  get_total_length(const std::vector<Eigen::Matrix<double, 7, 1>> &poses);
