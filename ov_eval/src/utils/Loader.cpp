...
void Loader::load_data(std::string path_traj, std::vector<double> &times,
  ...
  while (std::getline(file, current_line)) {
    ...
    // Loop through this line (timestamp(s) tx ty tz qx qy qz qw Pr11 Pr12 Pr13
    // Pr22 Pr23 Pr33 Pt11 Pt12 Pt13 Pt22 Pt23 Pt33)
    ...
    if (i >= 20) {
      ...
      c_ori << data(8), data(9), data(10), data(9), data(11), data(12),
          data(10), data(12), data(13);
      c_pos << data(14), data(15), data(16), data(15), data(17), data(18),
  ...
  if (times.size() != poses.size()) {
    PRINT_ERROR(
        RED
        "[LOAD]: Parsing error, pose and timestamps do not match!!\n" RESET);
    PRINT_ERROR(RED "[LOAD]: %s\n" RESET, path_traj.c_str());
    std::exit(EXIT_FAILURE);
  }
  ...
  // Assert that they are all equal
  if (!cov_ori.empty() &&
      (times.size() != cov_ori.size() || times.size() != cov_pos.size())) {
    PRINT_ERROR(RED "[LOAD]: Parsing error, timestamps covariance size do not "
                    "match!!\n" RESET);
    PRINT_ERROR(RED "[LOAD]: %s\n" RESET, path_traj.c_str());
    std::exit(EXIT_FAILURE);
  }
  ...
  // Debug print amount
  // std::string base_filename = path_traj.substr(path_traj.find_last_of("/\\")
  // + 1); PRINT_DEBUG("[LOAD]: loaded %d poses from
...
void Loader::load_data_csv(std::string path_traj, std::vector<double> &times,
  ...
  while (std::getline(file, current_line)) {
    ...
    // Loop through this line (groundtruth state
...
void Loader::load_simulation(std::string path,
  ...
  for (size_t i = 0; i < values.size(); i++) {
    if (values.at(i).rows() != rowsize) {
      PRINT_ERROR(RED "[LOAD]: Invalid row size on line %d (of size %d instead "
                      "of %d)\n" RESET,
                  (int)i, (int)values.at(i).rows(), rowsize);
      PRINT_ERROR(RED "[LOAD]: %s\n" RESET, path.c_str());
      std::exit(EXIT_FAILURE);
    }
  }
...
void Loader::load_timing_flamegraph(
  ...
  while (std::getline(file, current_line)) {
    ...
    // Here we will process them (skip the first since it is just the
    // timestamps)
    if (!current_line.find("#")) {
      // Loop variables
      std::istringstream s(current_line);
  ...
  for (size_t i = 0; i < timing_values.size(); i++) {
    if (timing_values.at(i).rows() != rowsize) {
      PRINT_ERROR(RED "[LOAD]: Invalid row size on line %d (of size %d instead "
                      "of %d)\n" RESET,
                  (int)i, (int)timing_values.at(i).rows(), rowsize);
      PRINT_ERROR(RED "[LOAD]: %s\n" RESET, path.c_str());
      std::exit(EXIT_FAILURE);
    }
  }
...
void Loader::load_timing_percent(std::string path, std::vector<double> &times,
  ...
  // Assert that they are all equal
  if (times.size() != summed_values.size() ||
      times.size() != node_values.size()) {
    PRINT_ERROR(
        RED
        "[LOAD]: Parsing error, pose and timestamps do not match!!\n" RESET);
    PRINT_ERROR(RED "[LOAD]: %s\n" RESET, path.c_str());
...
double Loader::get_total_length(
  ...
  for (size_t i = 1; i < poses.size(); i++) {
    distance +=
        (poses[i].block(0, 0, 3, 1) - poses[i - 1].block(0, 0, 3, 1)).norm();
  }
