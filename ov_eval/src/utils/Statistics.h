 ...
 *
 * Ensure that you call the calculate() function to update the values before
 * using them. This will compute all the final results from the values in @ref
 * values vector.
 */
struct Statistics {
...
public:
  ...
  void calculate() {
    ...
    if (values_sorted.size() == 1) {
      ...
      median = 0.5 * (values_sorted.at(values_sorted.size() / 2 - 1) +
                      values_sorted.at(values_sorted.size() / 2));
    } else {
      median = 0.0;
    }
