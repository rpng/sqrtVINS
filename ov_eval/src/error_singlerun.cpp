...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    PRINT_ERROR(RED "ERROR: Please specify a align mode, groudtruth, and "
                    "algorithm run file\n" RESET);
    PRINT_ERROR(RED "ERROR: ./error_singlerun <align_mode> <file_gt.txt> "
                    "<file_est.txt>\n" RESET);
    PRINT_ERROR(RED "ERROR: rosrun ov_eval error_singlerun <align_mode> "
                    "<file_gt.txt> <file_est.txt>\n" RESET);
  ...
  std::cout << "error_ori: " << error_ori.values.at(0) << std::endl;
  ...
  for (const auto &seg : error_rpe) {
    ...
    matplotlibcpp::boxplot(seg.second.first.values, ct++, width, "blue", "-",
                           params_rpe);
  }
  ...
  ct = 1;
  for (const auto &seg : error_rpe) {
    matplotlibcpp::boxplot(seg.second.second.values, ct++, width, "blue", "-",
                           params_rpe);
  }
  ...
  double endtime2 = (posx.timestamps.empty())
                        ? 0
                        : posx.timestamps.at(posx.timestamps.size() - 1);
