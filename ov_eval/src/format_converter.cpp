...
void process_csv(std::string infile) {
  ...
  if (boost::filesystem::exists(outfile)) {
    PRINT_ERROR(RED "\t- ERROR: Output file already exists, please delete and "
                    "re-run this script!!\n" RESET);
    PRINT_ERROR(RED "\t- ERROR: %s\n" RESET, outfile.c_str());
    return;
  }
  ...
  for (size_t i = 0; i < traj_data.size(); i++) {
    ...
    file2.precision(6);
    file2 << traj_data.at(i)(1) << " " << traj_data.at(i)(2) << " "
          << traj_data.at(i)(3) << " " << traj_data.at(i)(5) << " "
          << traj_data.at(i)(6) << " " << traj_data.at(i)(7) << " "
...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    PRINT_ERROR(
        RED
        "ERROR: rosrun ov_eval format_converter <file.csv or folder>\n" RESET);
    std::exit(EXIT_FAILURE);
  }
