...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < path_algorithms.size(); i++) {
    ...
    for (size_t j = 0; j < path_groundtruths.size(); j++) {
      ...
      ov_eval::Statistics gravity_dataset;
      ...
      for (auto &path_esttxt : file_paths) {
        ...
        if (!error_ori.values.empty())
          gravity_dataset.values.push_back(error_ori.values.at(0));
        // std::cout << error_ori.values.at(0) << std::endl;
      ...
      algo_gravity.at(algo).at(j) = gravity_dataset;
  ...
  PRINT_INFO("============================================\n");
  PRINT_INFO("SCALE ERROR LATEX TABLE\n");
  PRINT_INFO("============================================\n");
  for (size_t i = 0; i < path_groundtruths.size(); i++) {
  ...
  for (auto &algo : algo_scale) {
    ...
    for (auto &seg : algo.second) {
      if (seg.values.empty()) {
        ...
        seg.calculate();
        double scale_err = 100 * (std::max(seg.mean, 1 / seg.mean) - 1);
        PRINT_INFO(" & %.3f", scale_err);
        sum_scale += scale_err;
        sum_ct++;
      }
    }
    ...
    for (auto &seg : algo.second) {
      ...
      if (seg.first.values.empty() || seg.second.values.empty()) {
        ...
        for (size_t i = 0; i < seg.first.values.size(); i++) {
          if (seg.first.values[i] < ori_err && seg.second.values[i] < pos_err) {
            data_success_runs++;
            total_success_runs++;
            // std::cout << "1";
          } else {
  ...
  // Finally print the ATE for all the runs
  PRINT_INFO("============================================\n");
  PRINT_INFO("GRAVITY ERROR LATEX TABLE\n");
  PRINT_INFO("============================================\n");
  for (size_t i = 0; i < path_groundtruths.size(); i++) {
    std::string gtname = path_groundtruths.at(i).stem().string();
    boost::replace_all(gtname, "_", "\\_");
  ...
  for (auto &algo : algo_gravity) {
    ...
    int sum_ct = 0;
    for (auto &seg : algo.second) {
      if (seg.values.empty() || seg.values.empty()) {
        PRINT_INFO(" & -");
      } else {
        seg.calculate();
        PRINT_INFO(" & %.3f ", seg.mean);
        sum_ori += seg.mean;
        sum_ct++;
      }
    }
  ...
  for (auto &algo : algo_rpe) {
    ...
    for (auto &seg : algo.second) {
      xticks.push_back(ct_pos -
                       (algo_rpe.size() * (width + spacing) - width) / 2);
      if (is_init_window) {
        std::stringstream stream;
        stream << std::fixed << std::setprecision(1) << seg.first;
        labels.push_back(stream.str());
      } else {
        labels.push_back(std::to_string((int)seg.first));
      }
      matplotlibcpp::boxplot(seg.second.first.values, ct_pos, width,
                             colors.at(ct_algo % colors.size()),
  ...
  // Plot each RPE next to each other
  if (is_init_window) {
    matplotlibcpp::xlim(segments.at(0), segments.back() + segments.at(0));
  } else {
  ...
  for (auto &algo : algo_rpe) {
    ...
    // Loop through each length type
    for (auto &seg : algo.second) {
      matplotlibcpp::boxplot(seg.second.second.values, ct_pos, width,
                             colors.at(ct_algo % colors.size()),
                             linestyle.at(ct_algo / colors.size()), params_rpe);
      ct_pos += 1 + 3 * width;
    }
