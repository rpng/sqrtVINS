...
void AlignUtils::align_umeyama(
  ...
  std::vector<Eigen::Matrix<double, 3, 1>> model_zerocentered,
      data_zerocentered;
  ...
  // SVD decomposition
  Eigen::JacobiSVD<Eigen::Matrix<double, 3, 3>> svd(C, Eigen::ComputeFullV |
                                                           Eigen::ComputeFullU);
...
void AlignUtils::perform_association(
    double offset, double max_difference, std::vector<double> &est_times,
  ...
  AlignUtils::perform_association(offset, max_difference, est_times, gt_times,
                                  est_poses, gt_poses, est_covori, est_covpos,
                                  gt_covori, gt_covpos);
}
...
void AlignUtils::perform_association(
  ...
  std::vector<Eigen::Matrix3d> est_covori_temp, est_covpos_temp, gt_covori_temp,
      gt_covpos_temp;
  ...
  for (size_t i = 0; i < est_times.size(); i++) {
    ...
    while (gt_pointer < gt_times.size() &&
           ...
           std::abs(gt_times.at(gt_pointer) - (est_times.at(i) + offset)) >
               max_difference) {
      gt_pointer++;
    }
    ...
    while (gt_pointer < gt_times.size() &&
      ...
      if (std::abs(gt_times.at(gt_pointer) - (est_times.at(i) + offset)) >=
          best_diff) {
        break;
      }
  ...
  if (est_times.size() < 3) {
    PRINT_ERROR(RED "[ALIGN]: Was unable to associate groundtruth and estimate "
                    "trajectories\n" RESET);
