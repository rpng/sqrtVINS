 ...
 * patterns](https://ieeexplore.ieee.org/document/88573). This is what allows us
 * to find the transform between the two trajectories without worrying about
 * singularities for the absolute trajectory error.
 */
class AlignUtils {
...
public:
   ...
   * Equation (17)-(18) in [Zhang et al. 2018
   * IROS](http://rpg.ifi.uzh.ch/docs/IROS18_Zhang.pdf) paper.
   * @param C Data matrix
   */
   ...
   * @return Mean value
   */
  static inline Eigen::Matrix<double, 3, 1>
  get_mean(const std::vector<Eigen::Matrix<double, 3, 1>> &data) {
  ...
  /**
   * @brief Given a set of points in a model frame and a set of points in a data
   * frame, finds best transform between frames (subject to constraints).
   *
   ...
   * @param yaw_only Whether to only use yaw orientation (such as when frames
   * are already gravity aligned)
   */
  static void
  align_umeyama(const std::vector<Eigen::Matrix<double, 3, 1>> &data,
   ...
   */
  static void
  perform_association(double offset, double max_difference,
