...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    PRINT_ERROR(
        RED "ERROR: Please specify a timing and memory percent folder\n" RESET);
    PRINT_ERROR(RED "ERROR: ./timing_percentages <timings_folder>\n" RESET);
    PRINT_ERROR(
        RED
        "ERROR: rosrun ov_eval timing_percentages <timings_folder>\n" RESET);
  ...
  for (const auto &p : path_algorithms) {
    std::vector<ov_eval::Statistics> temp = {
        ov_eval::Statistics(), ov_eval::Statistics(), ov_eval::Statistics()};
  ...
  for (size_t i = 0; i < path_algorithms.size(); i++) {
    ...
    for (auto &entry : boost::filesystem::recursive_directory_iterator(
      ...
      ov_eval::Loader::load_timing_percent(entry.path().string(), times,
                                           summed_values, node_values);

      // Append to our summed values
      total_times.insert(total_times.end(), times.begin(), times.end());
      total_summed_values.insert(total_summed_values.end(),
                                 summed_values.begin(), summed_values.end());
    }
    ...
    for (size_t j = 0; j < total_times.size(); j++) {
      ...
      algo_timings.at(algo).at(0).values.push_back(
          total_summed_values.at(j)(0));
      algo_timings.at(algo).at(1).timestamps.push_back(total_times.at(j));
      algo_timings.at(algo).at(1).values.push_back(
          total_summed_values.at(j)(1));
      algo_timings.at(algo).at(2).timestamps.push_back(total_times.at(j));
      algo_timings.at(algo).at(2).values.push_back(
          total_summed_values.at(j)(2));
    }
    ...
    PRINT_DEBUG("\tPREC: mean_cpu = %.3f +- %.3f\n",
                algo_timings.at(algo).at(0).mean,
                algo_timings.at(algo).at(0).std);
    PRINT_DEBUG("\tPREC: mean_mem = %.3f +- %.3f\n",
                algo_timings.at(algo).at(1).mean,
                algo_timings.at(algo).at(1).std);
  ...
  for (auto &algo : algo_timings) {
    ...
    // Plot it!!!
    matplotlibcpp::boxplot(algo.second.at(0).values, ct_pos, width,
  ...
  for (auto &algo : algo_timings) {
    ...
    labels.push_back(algo.first);
    // Plot it!!!
    matplotlibcpp::boxplot(algo.second.at(1).values, ct_pos, width,
                           colors.at(ct_algo % colors.size()),
                           linestyle.at(ct_algo / colors.size()), params_rpe,
                           false);
    // Move forward
