...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    PRINT_ERROR(RED "ERROR: ./timing_comparison <file_times1.txt> ... "
                    "<file_timesN.txt>\n" RESET);
    PRINT_ERROR(RED "ERROR: rosrun ov_eval timing_comparison <file_times1.txt> "
                    "... <file_timesN.txt>\n" RESET);
  ...
  for (int z = 1; z < argc; z++) {
    ...
    ov_eval::Loader::load_timing_flamegraph(argv[z], names_temp, times,
                                            timing_values);
  ...
  // {"blue","aqua","lightblue","lightgreen","yellowgreen","green"};
  // std::vector<std::string> colors =
  // {"navy","blue","lightgreen","green","gold","goldenrod"};
  ...
  double starttime = (total_times.at(0).timestamps.empty())
                         ? 0
                         : total_times.at(0).timestamps.at(0);
  double endtime = (total_times.at(0).timestamps.empty())
                       ? 0
                       : total_times.at(0).timestamps.at(
                             total_times.at(0).timestamps.size() - 1);
