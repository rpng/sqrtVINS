 ...
 *
 * This class has all the error function which can be calculated for the loaded
 * trajectory. Given a groundtruth and trajectory we first align the two so that
 * they are in the same frame. From there the following errors could be
 * computed:
 ...
 *
 * Please see the @ref evaluation page for details and Zhang and Scaramuzza [A
 * Tutorial on Quantitative Trajectory Evaluation for Visual(-Inertial)
 * Odometry](http://rpg.ifi.uzh.ch/docs/IROS18_Zhang.pdf) paper for
 * implementation specific details.
 */
class ResultTrajectory {

public:
  double scale_est2gt = -1;

  /**
   * @brief Default constructor that will load, intersect, and align our
   * trajectories.
   * @param path_est Path to the estimate text file
   * @param path_gt Path to the groundtruth text file
   * @param alignment_method The alignment method to use [sim3, se3, posyaw,
   * none]
   */
  ResultTrajectory(std::string path_est, std::string path_gt,
   ...
   * \f{align*}{
   * e_{ATE} &= \sqrt{ \frac{1}{K} \sum_{k=1}^{K} ||\mathbf{x}_{k,i} \boxminus
   * \hat{\mathbf{x}}^+_{k,i}||^2_{2} } \f}
   *
  ...
  /**
   * @brief Computes the Absolute Trajectory Error (ATE) for this trajectory in
   * the 2d x-y plane.
   *
   * This will first do our alignment of the two trajectories.
   ...
   *
   * For the given set of segment lengths, this will split the trajectory into
   * segments. From there it will compute the relative pose error for all
   * segments. These are then returned as a map for each segment length.
   * \f{align*}{
   * \tilde{\mathbf{x}}_{r} &= \mathbf{x}_{k} \boxminus \mathbf{x}_{k+d_i} \\
   * e_{rpe,d_i} &= \frac{1}{D_i} \sum_{k=1}^{D_i} ||\tilde{\mathbf{x}}_{r}
   * \boxminus \hat{\tilde{\mathbf{x}}}_{r}||^2_{2} \f}
   *
   * @param segment_lengths What segment lengths we want to calculate for
   * @param error_rpe Map of segment lengths => errors for that length
   * (orientation and position)
   */
  void
  calculate_rpe(const std::vector<double> &segment_lengths,
                std::map<double, std::pair<Statistics, Statistics>> &error_rpe);

  /**
   * @brief Computes the Normalized Estimation Error Squared (NEES) for this
   * trajectory.
   *
   * If we have a covariance in addition to our pose estimate we can compute the
   * NEES values. At each timestep we compute this for both orientation and
   * position. \f{align*}{ e_{nees,k} &= \frac{1}{N} \sum_{i=1}^{N}
   * (\mathbf{x}_{k,i} \boxminus \hat{\mathbf{x}}_{k,i})^\top
   * \mathbf{P}^{-1}_{k,i}
   ...
   *
   * As compared to ATE error (see @ref calculate_ate()) this will compute the
   * error for each individual pose component. This is normally used if you just
   * want to look at a single run on a single dataset. \f{align*}{ e_{rmse,k} &=
   * \sqrt{ \frac{1}{N} \sum_{i=1}^{N} ||\mathbf{x}_{k,i} \boxminus
   * \hat{\mathbf{x}}_{k,i}||^2_{2} } \f}
   *
...
protected:
  ...
  /**
   * @brief Gets the indices at the end of subtractories of a given length when
   * starting at each index. For each starting pose, find the end pose index
   * which is the desired distance away.
   * @param distances Total distance travelled from start at each index
   * @param distance Distance of subtrajectory
   * @param max_dist_diff Maximum error between current trajectory length and
   * the desired
   * @return End indices for each subtrajectory
   */
  std::vector<int>
  compute_comparison_indices_length(std::vector<double> &distances,
                                    double distance, double max_dist_diff) {

    // Vector of end ids for our pose indexes
    std::vector<int> comparisons;

    // Loop through each pose in our trajectory (i.e. our distance vector
    // generated from the trajectory).
    for (size_t idx = 0; idx < distances.size(); idx++) {
      ...
      for (size_t i = idx; i < distances.size(); i++) {
        if (std::abs(distances.at(i) - (distance_startpose + distance)) <
            best_error) {
          best_idx = i;
          best_error =
              std::abs(distances.at(i) - (distance_startpose + distance));
        }
      }

      // If we have an end id that reached this trajectory distance then add it!
      // Else this isn't a valid segment, thus we shouldn't add it (we will try
      // again at the next pose) NOTE: just because we searched through all
      // poses and didn't find a close one doesn't mean we have ended NOTE: this
      // could happen if there is a gap in the groundtruth poses and we just
      // couldn't find a pose with low error
