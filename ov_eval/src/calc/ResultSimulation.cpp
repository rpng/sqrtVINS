...
ResultSimulation::ResultSimulation(std::string path_est, std::string path_std,
                                   std::string path_gt) {
...
void ResultSimulation::plot_state(bool doplotting, double max_time) {
  ...
  for (size_t i = 0; i < est_state.size(); i++) {
    ...
    Eigen::Matrix3d e_R =
        ov_core::quat_2_Rot(gt_state.at(i).block(1, 0, 4, 1)) *
    ...
    for (int j = 0; j < 3; j++) {
      ...
      error_ori[j].values_bound.push_back(3 * 180.0 / M_PI *
                                          state_cov.at(i)(1 + j));
    ...
    Eigen::Vector3d pos_err =
        gt_state.at(i).block(5, 0, 3, 1) - est_state.at(i).block(5, 0, 3, 1);
    for (int j = 0; j < 3; j++) {
    ...
    Eigen::Vector3d vel_err =
        gt_state.at(i).block(8, 0, 3, 1) - est_state.at(i).block(8, 0, 3, 1);
    for (int j = 0; j < 3; j++) {
    ...
    Eigen::Vector3d bg_err =
        gt_state.at(i).block(11, 0, 3, 1) - est_state.at(i).block(11, 0, 3, 1);
    for (int j = 0; j < 3; j++) {
    ...
    Eigen::Vector3d ba_err =
        gt_state.at(i).block(14, 0, 3, 1) - est_state.at(i).block(14, 0, 3, 1);
    for (int j = 0; j < 3; j++) {
...
void ResultSimulation::plot_timeoff(bool doplotting, double max_time) {
  ...
  for (size_t i = 0; i < est_state.size(); i++) {
    ...
    if (state_cov.at(i)(16) == 0.0) {
      PRINT_WARNING(
          YELLOW
          "Time offset was not calibrated online, so will not plot...\n" RESET);
      return;
    }
...
#ifndef HAVE_PYTHONLIBS
  PRINT_ERROR(
      RED "Matplotlib not loaded, so will not plot, just returning..\n" RESET);
  return;
#else
  ...
  // Zero our time array
  double starttime =
      (error_time.timestamps.empty()) ? 0 : error_time.timestamps.at(0);
  double endtime =
      (error_time.timestamps.empty())
          ? 0
          : error_time.timestamps.at(error_time.timestamps.size() - 1);
  ...
  if (!error_time.values_bound.empty()) {
    matplotlibcpp::plot(error_time.timestamps, error_time.values_bound,
                        params_bound);
    for (size_t i = 0; i < error_time.timestamps.size(); i++) {
...
void ResultSimulation::plot_cam_instrinsics(bool doplotting, double max_time) {
  ...
  if ((int)est_state.at(0)(18) < 1) {
    PRINT_ERROR(YELLOW
                "You need at least one camera to plot intrinsics...\n" RESET);
    return;
  }
  ...
  for (size_t i = 0; i < est_state.size(); i++) {
    ...
    if (state_cov.at(i)(18) == 0.0) {
      PRINT_WARNING(YELLOW "Camera intrinsics not calibrated online, so will "
                           "not plot...\n" RESET);
      return;
    }
    ...
    for (int n = 0; n < (int)est_state.at(0)(18); n++) {
      for (int j = 0; j < 4; j++) {
        ...
        error_cam_k.at(n).at(j).values.push_back(
            est_state.at(i)(19 + 15 * n + j) - gt_state.at(i)(19 + 15 * n + j));
        error_cam_k.at(n).at(j).values_bound.push_back(
            3 * state_cov.at(i)(18 + 14 * n + j));
        error_cam_d.at(n).at(j).timestamps.push_back(est_state.at(i)(0));
        error_cam_d.at(n).at(j).values.push_back(
            est_state.at(i)(19 + 4 + 15 * n + j) -
            gt_state.at(i)(19 + 4 + 15 * n + j));
        error_cam_d.at(n).at(j).values_bound.push_back(
            3 * state_cov.at(i)(18 + 4 + 14 * n + j));
      }
    }
  }
  ...
  // Plot line colors
  std::vector<std::string> colors = {"blue",  "red",  "black",
                                     "green", "cyan", "magenta"};
  ...
  for (int n = 0; n < (int)est_state.at(0)(18); n++) {
    std::string estcolor =
        ((int)est_state.at(0)(18) == 1) ? "blue" : colors.at(n);
    std::string stdcolor =
        ((int)est_state.at(0)(18) == 1) ? "red" : colors.at(n);
    plot_4errors(error_cam_k.at(n)[0], error_cam_k.at(n)[1],
                 error_cam_k.at(n)[2], error_cam_k.at(n)[3], colors.at(n),
                 stdcolor);
  }
  ...
  for (int n = 0; n < (int)est_state.at(0)(18); n++) {
    ...
    std::string stdcolor =
        ((int)est_state.at(0)(18) == 1) ? "red" : colors.at(n);
    plot_4errors(error_cam_d.at(n)[0], error_cam_d.at(n)[1],
                 error_cam_d.at(n)[2], error_cam_d.at(n)[3], estcolor,
                 stdcolor);
  }
...
void ResultSimulation::plot_cam_extrinsics(bool doplotting, double max_time) {
  ...
  for (size_t i = 0; i < est_state.size(); i++) {
    ...
    if (state_cov.at(i)(26) == 0.0) {
      PRINT_WARNING(YELLOW "Camera extrinsics not calibrated online, so will "
                           "not plot...\n" RESET);
      return;
    }
    ...
    for (int n = 0; n < (int)est_state.at(0)(18); n++) {
      // NOTE: we define our error as e_R = -Log(R*Rhat^T)
      Eigen::Matrix3d e_R =
          ov_core::quat_2_Rot(gt_state.at(i).block(27 + 15 * n, 0, 4, 1)) *
          ov_core::quat_2_Rot(est_state.at(i).block(27 + 15 * n, 0, 4, 1))
              .transpose();
      Eigen::Vector3d ori_err = -180.0 / M_PI * ov_core::log_so3(e_R);
      // Eigen::Matrix3d e_R =
      // Math::quat_2_Rot(est_state.at(i).block(27+15*n,0,4,1)).transpose() *
      // Math::quat_2_Rot(gt_state.at(i).block(27+15*n,0,4,1)); Eigen::Vector3d
      // ori_err = 180.0/M_PI*Math::log_so3(e_R);
      for (int j = 0; j < 3; j++) {
        error_cam_ori.at(n).at(j).timestamps.push_back(est_state.at(i)(0));
        error_cam_ori.at(n).at(j).values.push_back(ori_err(j));
        error_cam_ori.at(n).at(j).values_bound.push_back(
            3 * 180.0 / M_PI * state_cov.at(i)(26 + 14 * n + j));
        error_cam_pos.at(n).at(j).timestamps.push_back(est_state.at(i)(0));
        error_cam_pos.at(n).at(j).values.push_back(
            est_state.at(i)(27 + 4 + 15 * n + j) -
            gt_state.at(i)(27 + 4 + 15 * n + j));
        error_cam_pos.at(n).at(j).values_bound.push_back(
            3 * state_cov.at(i)(26 + 3 + 14 * n + j));
      }
    }
  }
  ...
  for (int n = 0; n < (int)est_state.at(0)(18); n++) {
    ...
    std::string stdcolor =
        ((int)est_state.at(0)(18) == 1) ? "red" : colors.at(n);
    plot_3errors(error_cam_ori.at(n)[0], error_cam_ori.at(n)[1],
                 error_cam_ori.at(n)[2], colors.at(n), stdcolor);
  }
  ...
  for (int n = 0; n < (int)est_state.at(0)(18); n++) {
    ...
    std::string stdcolor =
        ((int)est_state.at(0)(18) == 1) ? "red" : colors.at(n);
    plot_3errors(error_cam_pos.at(n)[0], error_cam_pos.at(n)[1],
                 error_cam_pos.at(n)[2], estcolor, stdcolor);
  }
...
void ResultSimulation::plot_imu_intrinsics(bool doplotting, double max_time) {
  ...
  // Check that we have cameras
  if ((int)est_state.at(0)(18) < 1) {
    PRINT_ERROR(YELLOW "You need at least one camera to run estimator and plot "
                       "results...\n" RESET);
    return;
  }
  ...
  for (size_t i = 0; i < est_state.size(); i++) {
    ...
    if (state_cov.at(i)(dw_cov_id) == 0.0) {
      PRINT_WARNING(
          YELLOW
          "IMU intrinsics not calibrated online, so will not plot...\n" RESET);
      return;
    }
    ...
    for (int j = 0; j < 6; j++) {
      ...
      error_dw.at(j).values.push_back(est_state.at(i)(dw_id + j) -
      ...
      error_da.at(j).values.push_back(est_state.at(i)(da_id + j) -
    ...
    for (int j = 0; j < 9; j++) {
      ...
      error_tg.at(j).values.push_back(est_state.at(i)(tg_id + j) -
                                      gt_state.at(i)(tg_id + j));
      error_tg.at(j).values_bound.push_back(3 * state_cov.at(i)(tg_cov_id + j));
    }

    // Calculate orientation errors
    Eigen::Matrix3d e_R_wtoI =
        ov_core::quat_2_Rot(gt_state.at(i).block<4, 1>(wtoI_id, 0)) *
        ov_core::quat_2_Rot(est_state.at(i).block<4, 1>(wtoI_id, 0))
            .transpose();
    Eigen::Vector3d ori_wtoI = -180.0 / M_PI * ov_core::log_so3(e_R_wtoI);
    Eigen::Matrix3d e_R_atoI =
        ov_core::quat_2_Rot(gt_state.at(i).block<4, 1>(atoI_id, 0)) *
        ov_core::quat_2_Rot(est_state.at(i).block<4, 1>(atoI_id, 0))
            .transpose();
    ...
    for (int j = 0; j < 3; j++) {
      ...
      error_wtoI.at(j).values_bound.push_back(3 * 180.0 / M_PI *
      ...
      error_atoI.at(j).values_bound.push_back(3 * 180.0 / M_PI *
  ...
  std::vector<std::string> colors = {"blue",  "red",  "black",
                                     "green", "cyan", "magenta"};
  std::string estcolor =
      ((int)est_state.at(0)(18) == 1) ? "blue" : colors.at(0);
  std::string stdcolor = ((int)est_state.at(0)(18) == 1) ? "red" : colors.at(1);

  // Plot this figure
  matplotlibcpp::figure_size(1000, 500);
  plot_6errors(error_dw[0], error_dw[3], error_dw[1], error_dw[4], error_dw[2],
               error_dw[5], colors.at(0), stdcolor);
  ...
  // Plot this figure
  matplotlibcpp::figure_size(1000, 500);
  plot_6errors(error_da[0], error_da[3], error_da[1], error_da[4], error_da[2],
               error_da[5], colors.at(0), stdcolor);
  ...
  plot_9errors(error_tg[0], error_tg[3], error_tg[6], error_tg[1], error_tg[4],
               error_tg[7], error_tg[2], error_tg[5], error_tg[8], colors.at(0),
               stdcolor);
  ...
  if (imu_model == 0) {
    ...
    // Plot this figure
    matplotlibcpp::figure_size(800, 500);
    plot_3errors(error_wtoI[0], error_wtoI[1], error_wtoI[2], colors.at(0),
                 stdcolor);
    ...
    // Plot this figure
    matplotlibcpp::figure_size(800, 500);
    plot_3errors(error_atoI[0], error_atoI[1], error_atoI[2], colors.at(0),
                 stdcolor);
