 ...
 *
 * This should match the recording logic that is in the ov_msckf::RosVisualizer
 * in which we write both estimate, their deviation, and groundtruth to three
 * files. We enforce that these files first contain the current IMU state, then
 * time offset, number of cameras, then the camera calibration states. If we are
 * not performing calibration these should all be written to file, just their
 * deviation should be zero as they are 100% certain.
 */
...
public:
   ...
   */
  ResultSimulation(std::string path_est, std::string path_std,
                   std::string path_gt);
...
protected:
   ...
   */
  void plot_3errors(ov_eval::Statistics sx, ov_eval::Statistics sy,
                    ov_eval::Statistics sz, std::string color_err,
                    std::string color_std) {

    // Zero our time arrays
    double starttime1 = (sx.timestamps.empty()) ? 0 : sx.timestamps.at(0);
    double endtime1 = (sx.timestamps.empty())
                          ? 0
                          : sx.timestamps.at(sx.timestamps.size() - 1);
    ...
    double endtime2 = (sy.timestamps.empty())
                          ? 0
                          : sy.timestamps.at(sy.timestamps.size() - 1);
    ...
    double endtime3 = (sz.timestamps.empty())
                          ? 0
                          : sz.timestamps.at(sz.timestamps.size() - 1);
   ...
   * @param color_std MATLAB color string for deviation (blue, red, etc.)
   */
  void plot_4errors(ov_eval::Statistics sx, ov_eval::Statistics sy,
                    ov_eval::Statistics sz, ov_eval::Statistics sk,
    ...
    double endtime4 = (sk.timestamps.empty())
                          ? 0
                          : sk.timestamps.at(sk.timestamps.size() - 1);
   ...
   * @param color_std MATLAB color string for deviation (blue, red, etc.)
   */
  void plot_6errors(ov_eval::Statistics s1, ov_eval::Statistics s2,
                    ov_eval::Statistics s3, ov_eval::Statistics s4,
                    ov_eval::Statistics s5, ov_eval::Statistics s6,
                    std::string color_err, std::string color_std) {

    // Zero our time arrays
    double starttime1 = (s1.timestamps.empty()) ? 0 : s1.timestamps.at(0);
    double endtime1 = (s1.timestamps.empty())
                          ? 0
                          : s1.timestamps.at(s1.timestamps.size() - 1);
    ...
    double endtime2 = (s2.timestamps.empty())
                          ? 0
                          : s2.timestamps.at(s2.timestamps.size() - 1);
    ...
    double endtime3 = (s3.timestamps.empty())
                          ? 0
                          : s3.timestamps.at(s3.timestamps.size() - 1);
    ...
    double endtime4 = (s4.timestamps.empty())
                          ? 0
                          : s4.timestamps.at(s4.timestamps.size() - 1);
    ...
    double endtime5 = (s5.timestamps.empty())
                          ? 0
                          : s5.timestamps.at(s5.timestamps.size() - 1);
    ...
    double endtime6 = (s6.timestamps.empty())
                          ? 0
                          : s6.timestamps.at(s6.timestamps.size() - 1);
   ...
   * @param color_std MATLAB color string for deviation (blue, red, etc.)
   */
  void plot_9errors(ov_eval::Statistics s1, ov_eval::Statistics s2,
                    ov_eval::Statistics s3, ov_eval::Statistics s4,
                    ov_eval::Statistics s5, ov_eval::Statistics s6,
                    ov_eval::Statistics s7, ov_eval::Statistics s8,
                    ov_eval::Statistics s9, std::string color_err,
                    std::string color_std) {
    ...
    double endtime7 = (s7.timestamps.empty())
                          ? 0
                          : s7.timestamps.at(s7.timestamps.size() - 1);
    ...
    double endtime8 = (s8.timestamps.empty())
                          ? 0
                          : s8.timestamps.at(s8.timestamps.size() - 1);
    ...
    double endtime9 = (s9.timestamps.empty())
                          ? 0
                          : s9.timestamps.at(s9.timestamps.size() - 1);
