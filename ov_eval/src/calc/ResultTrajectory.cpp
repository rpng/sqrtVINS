...
ResultTrajectory::ResultTrajectory(std::string path_est, std::string path_gt,
  ...
  // Debug print amount
  // std::string base_filename1 = path_est.substr(path_est.find_last_of("/\\") +
  // 1); std::string base_filename2 = path_gt.substr(path_gt.find_last_of("/\\")
  // + 1); PRINT_DEBUG("[TRAJ]: loaded %d poses from
  // %s\n",(int)est_times.size(),base_filename1.c_str()); PRINT_DEBUG("[TRAJ]:
  // loaded %d poses from %s\n",(int)gt_times.size(),base_filename2.c_str());

  // Intersect timestamps
  // PRINT_ERROR(RED "[TRAJ]: num estimated poses in %s before association:
  // %d.\n" RESET, path_est.c_str(), (int)est_poses.size());
  AlignUtils::perform_association(0, 0.02, est_times, gt_times, est_poses,
                                  gt_poses, est_covori, est_covpos, gt_covori,
                                  gt_covpos);

  // Fail if we didn't have any common timestamps
  is_valid_ = est_poses.size() > 2;

  if (!is_valid()) {
    PRINT_WARNING(RED "[TRAJ]: unable to get enough common timestamps between "
                      "trajectory %s and gt %s (matched %d).\n" RESET,
                  path_est.c_str(), path_gt.c_str(), (int)est_poses.size());
    PRINT_WARNING(RED "[TRAJ]: does the estimated trajectory publish the "
                      "rosbag timestamps??\n" RESET);
  } else {
    ...
    AlignTrajectory::align_trajectory(est_poses, gt_poses, R_ESTtoGT, t_ESTinGT,
                                      s_ESTtoGT, alignment_method);
    AlignTrajectory::align_trajectory(gt_poses, est_poses, R_GTtoEST, t_GTinEST,
                                      s_GTtoEST, alignment_method);
    ...
    PRINT_DEBUG("[TRAJ]: q_ESTtoGT = %.3f, %.3f, %.3f, %.3f | p_ESTinGT = "
                "%.3f, %.3f, %.3f | s = %.2f\n",
                q_ESTtoGT(0), q_ESTtoGT(1), q_ESTtoGT(2), q_ESTtoGT(3),
                t_ESTinGT(0), t_ESTinGT(1), t_ESTinGT(2), s_ESTtoGT);
    // PRINT_DEBUG("[TRAJ]: q_GTtoEST = %.3f, %.3f, %.3f, %.3f | p_GTinEST =
    // %.3f, %.3f, %.3f | s =
    // %.2f\n",q_GTtoEST(0),q_GTtoEST(1),q_GTtoEST(2),q_GTtoEST(3),t_GTinEST(0),t_GTinEST(1),t_GTinEST(2),s_GTtoEST);

    scale_est2gt = s_ESTtoGT;
    ...
    for (size_t i = 0; i < gt_times.size(); i++) {
      ...
      pose_ESTinGT.block(0, 0, 3, 1) =
          s_ESTtoGT * R_ESTtoGT * est_poses.at(i).block(0, 0, 3, 1) + t_ESTinGT;
      pose_ESTinGT.block(3, 0, 4, 1) = ov_core::quat_multiply(
          est_poses.at(i).block(3, 0, 4, 1), ov_core::Inv(q_ESTtoGT));
      pose_GTinEST.block(0, 0, 3, 1) =
          s_GTtoEST * R_GTtoEST * gt_poses.at(i).block(0, 0, 3, 1) + t_GTinEST;
      pose_GTinEST.block(3, 0, 4, 1) = ov_core::quat_multiply(
          gt_poses.at(i).block(3, 0, 4, 1), ov_core::Inv(q_GTtoEST));
...
void ResultTrajectory::calculate_ate(Statistics &error_ori,
  ...
  if (!is_valid()) {
    error_ori.timestamps.push_back(0);
    error_ori.values.push_back(0);
    error_pos.timestamps.push_back(0);
    error_pos.values.push_back(0);
  }
  ...
  for (size_t i = 0; i < est_poses_aignedtoGT.size(); i++) {
    ...
    Eigen::Matrix3d e_R =
        ov_core::quat_2_Rot(est_poses_aignedtoGT.at(i).block(3, 0, 4, 1))
            .transpose() *
        ov_core::quat_2_Rot(gt_poses.at(i).block(3, 0, 4, 1));
    double ori_err = 180.0 / M_PI * ov_core::log_so3(e_R).norm();

    // Calculate position error
    double pos_err = (gt_poses.at(i).block(0, 0, 3, 1) -
                      est_poses_aignedtoGT.at(i).block(0, 0, 3, 1))
                         .norm();
...
void ResultTrajectory::calculate_ate_2d(Statistics &error_ori,
  ...
  for (size_t i = 0; i < est_poses_aignedtoGT.size(); i++) {
    ...
    // Calculate position error
    double pos_err = (gt_poses.at(i).block(0, 0, 2, 1) -
                      est_poses_aignedtoGT.at(i).block(0, 0, 2, 1))
                         .norm();
...
void ResultTrajectory::calculate_rpe(
  ...
  for (size_t i = 1; i < gt_poses.size(); i++) {
    double pos_diff =
        (gt_poses[i].block(0, 0, 3, 1) - gt_poses[i - 1].block(0, 0, 3, 1))
            .norm();
  ...
  if (average_pos_difference > max_dist_diff) {
    PRINT_WARNING(YELLOW "[COMP]: average groundtruth position difference %.2f "
                         "> %.2f is too large\n" RESET,
                  average_pos_difference, max_dist_diff);
    PRINT_WARNING(YELLOW "[COMP]: this will prevent the RPE from finding valid "
                         "trajectory segments!!!\n" RESET);
    PRINT_WARNING(
        YELLOW
        "[COMP]: the recommendation is to use a higher frequency groundtruth, "
        "or relax this trajectory segment logic...\n" RESET);
  }
  ...
  for (const double &distance : segment_lengths) {
    ...
    // NOTE: is there a better way to select which end pos is a valid segments
    // that are of the correct lenght? NOTE: right now this allows for longer
    // segments to have bigger error in their start-end distance vs the desired
    // segment length std::vector<int> comparisons =
    // compute_comparison_indices_length(accum_distances, distance,
    // 0.1*distance);
    std::vector<int> comparisons = compute_comparison_indices_length(
        accum_distances, distance, max_dist_diff);
    ...
    for (size_t id_start = 0; id_start < comparisons.size(); id_start++) {
      ...
      T_c1.block(0, 0, 3, 3) =
          ov_core::quat_2_Rot(
              est_poses_aignedtoGT.at(id_start).block(3, 0, 4, 1))
              .transpose();
      T_c1.block(0, 3, 3, 1) =
          est_poses_aignedtoGT.at(id_start).block(0, 0, 3, 1);

      // Get T I2 to world EST at end of subtrajectory starting (at state
      // comparisons[idx])
      Eigen::Matrix4d T_c2 = Eigen::Matrix4d::Identity();
      T_c2.block(0, 0, 3, 3) =
          ov_core::quat_2_Rot(est_poses_aignedtoGT.at(id_end).block(3, 0, 4, 1))
              .transpose();
      T_c2.block(0, 3, 3, 1) =
          est_poses_aignedtoGT.at(id_end).block(0, 0, 3, 1);
      ...
      T_m1.block(0, 0, 3, 3) =
          ov_core::quat_2_Rot(gt_poses.at(id_start).block(3, 0, 4, 1))
              .transpose();
      T_m1.block(0, 3, 3, 1) = gt_poses.at(id_start).block(0, 0, 3, 1);

      // Get T I2 to world GT at end of subtrajectory starting (at state
      // comparisons[idx])
      Eigen::Matrix4d T_m2 = Eigen::Matrix4d::Identity();
      T_m2.block(0, 0, 3, 3) =
          ov_core::quat_2_Rot(gt_poses.at(id_end).block(3, 0, 4, 1))
              .transpose();
      ...
      // Rotate rotation so that rotation error is in the global frame (allows
      // us to look at yaw error)
...
void ResultTrajectory::calculate_nees(Statistics &nees_ori,
  ...
  if (est_times.size() != est_covori.size() ||
      est_times.size() != est_covpos.size() ||
      gt_times.size() != gt_covori.size() ||
      gt_times.size() != gt_covpos.size()) {
    PRINT_WARNING(YELLOW
                  "[TRAJ]: Normalized Estimation Error Squared called but "
                  "trajectory does not have any covariances...\n" RESET);
    PRINT_WARNING(YELLOW "[TRAJ]: Did you record using a Odometry or "
  ...
  for (size_t i = 0; i < est_poses.size(); i++) {
    ...
    Eigen::Matrix3d e_R =
        ov_core::quat_2_Rot(gt_poses_aignedtoEST.at(i).block(3, 0, 4, 1)) *
    ...
    if (std::isnan(ori_nees) || std::isnan(pos_nees)) {
      PRINT_WARNING(YELLOW "[TRAJ]: nees calculation had nan number "
                           "(covariance is wrong?) skipping...\n" RESET);
      continue;
    }
...
void ResultTrajectory::calculate_error(Statistics &posx, Statistics &posy,
                                       Statistics &posz, Statistics &orix,
                                       Statistics &oriy, Statistics &oriz,
                                       Statistics &roll, Statistics &pitch,
                                       Statistics &yaw) {
  ...
  for (size_t i = 0; i < est_poses.size(); i++) {
    ...
    // Calculate local orientation error, then propagate its covariance into the
    // global frame of reference
    Eigen::Vector4d e_q =
        ov_core::quat_multiply(gt_poses_aignedtoEST.at(i).block(3, 0, 4, 1),
                               ov_core::Inv(est_poses.at(i).block(3, 0, 4, 1)));
    Eigen::Vector3d errori_local = 2 * e_q.block(0, 0, 3, 1);
    Eigen::Vector3d errori_global =
        ov_core::quat_2_Rot(est_poses.at(i).block(3, 0, 4, 1)).transpose() *
        errori_local;
    ...
    // NOTE: this error is in the "global" frame since we do rot2rpy on R_ItoG
    // rotation
    Eigen::Vector3d ypr_est_ItoG = ov_core::rot2rpy(
        ov_core::quat_2_Rot(est_poses.at(i).block(3, 0, 4, 1)).transpose());
    Eigen::Vector3d ypr_gt_ItoG = ov_core::rot2rpy(
        ov_core::quat_2_Rot(gt_poses_aignedtoEST.at(i).block(3, 0, 4, 1))
            .transpose());
    ...
    // NOTE: example can be found here
    // http://mars.cs.umn.edu/tr/reports/Trawny05c.pdf
    Eigen::Matrix<double, 3, 3> H_xyz2rpy =
        Eigen::Matrix<double, 3, 3>::Identity();
    H_xyz2rpy.block(0, 1, 3, 1) =
        ov_core::rot_x(-ypr_est_ItoG(0)) * H_xyz2rpy.block(0, 1, 3, 1);
    H_xyz2rpy.block(0, 2, 3, 1) = ov_core::rot_x(-ypr_est_ItoG(0)) *
                                  ov_core::rot_y(-ypr_est_ItoG(1)) *
                                  H_xyz2rpy.block(0, 2, 3, 1);
    Eigen::Matrix3d cov_rpy;
    if (est_times.size() == est_covori.size()) {
      cov_rpy = H_xyz2rpy.inverse() * est_covori.at(i) *
                H_xyz2rpy.inverse().transpose();
    }
    ...
    if (est_times.size() == est_covori.size()) {
      orix.values_bound.push_back(3 * 180.0 / M_PI *
                                  std::sqrt(cov_global(0, 0)));
      oriy.values_bound.push_back(3 * 180.0 / M_PI *
                                  std::sqrt(cov_global(1, 1)));
      oriz.values_bound.push_back(3 * 180.0 / M_PI *
                                  std::sqrt(cov_global(2, 2)));
    }
