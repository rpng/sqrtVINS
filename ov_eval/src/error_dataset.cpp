...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    PRINT_ERROR(
        RED
        "ERROR: Please specify a align mode, folder, and algorithms\n" RESET);
    PRINT_ERROR(RED "ERROR: ./error_dataset <align_mode> <file_gt.txt> "
                    "<folder_algorithms>\n" RESET);
    PRINT_ERROR(RED "ERROR: rosrun ov_eval error_dataset <align_mode> "
                    "<file_gt.txt> <folder_algorithms>\n" RESET);
  ...
  std::map<std::string, std::pair<ov_eval::Statistics, ov_eval::Statistics>>
      algo_ate;
  std::map<std::string, std::pair<ov_eval::Statistics, ov_eval::Statistics>>
      algo_nees;
  for (const auto &p : path_algorithms) {
    algo_ate.insert({p.filename().string(),
                     {ov_eval::Statistics(), ov_eval::Statistics()}});
    algo_nees.insert({p.filename().string(),
                      {ov_eval::Statistics(), ov_eval::Statistics()}});
  }
  ...
  for (size_t i = 0; i < path_algorithms.size(); i++) {
    ...
    if (path_algo_datasets.find(path_gt.stem().string()) ==
    ...
    for (auto &entry : boost::filesystem::directory_iterator(
             path_algo_datasets.at(path_gt.stem().string()))) {
    ...
    if (file_paths.empty()) {
      PRINT_DEBUG(RED "\tERROR: No runs found for %s, is the folder structure "
                      "right??\n" RESET,
                  path_algorithms.at(i).filename().c_str());
      continue;
    }
    ...
    for (auto &path_esttxt : file_paths) {
      ...
      for (size_t j = 0; j < error_ori.values.size(); j++) {
        rmse_dataset[error_ori.timestamps.at(j)].first.values.push_back(
            error_ori.values.at(j));
        rmse_dataset[error_pos.timestamps.at(j)].second.values.push_back(
            error_pos.values.at(j));
      ...
      for (size_t j = 0; j < error_ori_2d.values.size(); j++) {
        rmse_2d_dataset[error_ori_2d.timestamps.at(j)].first.values.push_back(
            error_ori_2d.values.at(j));
        rmse_2d_dataset[error_pos_2d.timestamps.at(j)].second.values.push_back(
            error_pos_2d.values.at(j));
      ...
      for (size_t j = 0; j < nees_ori.values.size(); j++) {
        nees_dataset[nees_ori.timestamps.at(j)].first.values.push_back(
            nees_ori.values.at(j));
        nees_dataset[nees_ori.timestamps.at(j)].second.values.push_back(
            nees_pos.values.at(j));
    ...
    for (auto &seg : rpe_dataset) {
      ...
      PRINT_DEBUG(
          ...
          (int)seg.first, seg.second.first.mean, seg.second.second.mean,
    ...
    // RMSE: Convert into the right format (only use times where all runs have
    // an error)
    ...
    // NEES: Convert into the right format (only use times where all runs have
    // an error)
    ov_eval::Statistics nees_ori, nees_pos;
    ...
    double starttime1 =
        (rmse_ori.timestamps.empty()) ? 0 : rmse_ori.timestamps.at(0);
    double endtime1 =
        (rmse_ori.timestamps.empty())
            ? 0
            : rmse_ori.timestamps.at(rmse_ori.timestamps.size() - 1);
    ...
    matplotlibcpp::title("Root Mean Squared Error - " +
                         path_algorithms.at(i).filename().string());
    ...
    if (!nees_ori.values.empty() && !nees_pos.values.empty()) {
      ...
      // Zero our time arrays
      double starttime2 =
          (nees_ori.timestamps.empty()) ? 0 : nees_ori.timestamps.at(0);
      double endtime2 =
          (nees_ori.timestamps.empty())
              ? 0
              : nees_ori.timestamps.at(nees_ori.timestamps.size() - 1);
  ...
  for (auto &algo : algo_ate) {
    ...
    if (ate_oripos.first.values.empty() || ate_oripos.second.values.empty()) {
      ...
      PRINT_INFO(" & %.3f / %.3f", ate_oripos.first.mean,
                 ate_oripos.second.mean);
    }
    // nees
    ...
    if (nees_oripos.first.values.empty() || nees_oripos.second.values.empty()) {
      ...
      PRINT_INFO(" & %.3f / %.3f", nees_oripos.first.mean,
                 nees_oripos.second.mean);
    }
    PRINT_INFO(" \\\\\n");
  }
  PRINT_INFO("============================================\n");
  ...
  for (auto &algo : algo_rpe) {
    ...
    for (auto &seg : algo.second) {
      seg.second.first.calculate();
      seg.second.second.calculate();
      PRINT_INFO(" & %.3f / %.3f", seg.second.first.mean,
                 seg.second.second.mean);
    }
    PRINT_INFO(" \\\\\n");
  }
  PRINT_INFO("============================================\n");
