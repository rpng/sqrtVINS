...
int main(int argc, char **argv) {
  ...
  if (infolder.extension() == ".csv") {
    ...
    ov_eval::Loader::load_data_csv(path_to_gt, times_gt, poses_gt, cov_ori_temp,
                                   cov_pos_temp);
  } else {
    std::vector<Eigen::Matrix3d> cov_ori_temp, cov_pos_temp;
    ov_eval::Loader::load_data(path_to_gt, times_gt, poses_gt, cov_ori_temp,
                               cov_pos_temp);
  }

  // Our subscribe and publish nodes
  std::string topic;
  nh.param<std::string>("sub_topic", topic, "/ov_srekf/pathimu");

  ros::Subscriber sub = nh.subscribe(topic, 1, align_and_publish);
  size_t pos = topic.find("pathimu");
  std::string pub_topic;
  if (pos != std::string::npos) {
    topic.replace(pos, std::string("pathimu").length(), "pathgt");
  }
  pub_path = nh.advertise<nav_msgs::Path>(topic, 2);
...
void align_and_publish(const nav_msgs::Path::ConstPtr &msg) {
  ...
  for (auto const &pose : msg->poses) {
    ...
    pose_tmp << pose.pose.position.x, pose.pose.position.y,
  ...
  ov_eval::AlignUtils::perform_association(0, 0.02, times_temp, gt_times_temp,
                                           poses_temp, gt_poses_temp);

  // Return failure if we didn't have any common timestamps
  if (poses_temp.size() < 3) {
    PRINT_ERROR(RED "[TRAJ]: unable to get enough common timestamps between "
                    "trajectories.\n" RESET);
    PRINT_ERROR(RED "[TRAJ]: does the estimated trajectory publish the rosbag "
                    "timestamps??\n" RESET);
    return;
  }
  ...
  ov_eval::AlignTrajectory::align_trajectory(poses_temp, gt_poses_temp,
                                             R_ESTtoGT, t_ESTinGT, s_ESTtoGT,
                                             alignment_type);
  Eigen::Vector4d q_ESTtoGT = ov_core::rot_2_quat(R_ESTtoGT);
  PRINT_DEBUG("[TRAJ]: q_ESTtoGT = %.3f, %.3f, %.3f, %.3f | p_ESTinGT = %.3f, "
              "%.3f, %.3f | s = %.2f\n",
  ...
  for (size_t i = 0; i < gt_times_temp.size();
       i += std::floor(gt_times_temp.size() / 16384.0) + 1) {

    // Convert into the correct frame
    double timestamp = gt_times_temp.at(i);
    Eigen::Matrix<double, 7, 1> pose_inGT = gt_poses_temp.at(i);
    Eigen::Vector3d pos_IinEST = R_ESTtoGT.transpose() *
                                 (pose_inGT.block(0, 0, 3, 1) - t_ESTinGT) /
                                 s_ESTtoGT;
    Eigen::Vector4d quat_ESTtoI =
        ov_core::quat_multiply(pose_inGT.block(3, 0, 4, 1), q_ESTtoGT);
    // Finally push back
