...
void plot_z_positions(const std::string &name, const std::string &color,
...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    ...
    PRINT_ERROR(RED "ERROR: ./plot_trajectories <align_mode> <file_gt.txt> "
                    "<file_est1.txt> ...  <file_est9.txt>\n" RESET);
    PRINT_ERROR(RED
                "ERROR: rosrun ov_eval plot_trajectories <align_mode> "
                "<file_gt.txt> <file_est1.txt> ...  <file_est9.txt>\n" RESET);
  ...
  for (int i = 2; i < argc; i++) {
    ...
    if (i > 2) {
      ...
      ov_eval::AlignUtils::perform_association(
          0, 0.02, times_temp, gt_times_temp, poses_temp, gt_poses_temp);

      // Return failure if we didn't have any common timestamps
      if (poses_temp.size() < 3) {
        PRINT_ERROR(RED "[TRAJ]: unable to get enough common timestamps "
                        "between trajectories.\n" RESET);
        PRINT_ERROR(RED "[TRAJ]: does the estimated trajectory publish the "
                        "rosbag timestamps??\n" RESET);
        std::exit(EXIT_FAILURE);
      }
      ...
      double s_ESTtoGT;
      ov_eval::AlignTrajectory::align_trajectory(
          poses_temp, gt_poses_temp, R_ESTtoGT, t_ESTinGT, s_ESTtoGT, argv[1]);
      ...
      for (size_t j = 0; j < gt_times_temp.size(); j++) {
        ...
        pose_ESTinGT.block(0, 0, 3, 1) =
            s_ESTtoGT * R_ESTtoGT * poses_temp.at(j).block(0, 0, 3, 1) +
            t_ESTinGT;
        pose_ESTinGT.block(3, 0, 4, 1) = ov_core::quat_multiply(
            poses_temp.at(j).block(3, 0, 4, 1), ov_core::Inv(q_ESTtoGT));
  ...
  std::vector<std::string> colors = {"black", "blue", "red",
                                     "green", "cyan", "magenta"};
  ...
  double endtime =
      (times.at(0).empty()) ? 0 : times.at(0).at(times.at(0).size() - 1);
  for (size_t i = 0; i < times.size(); i++) {
