...
public:
   ...
   * points (timestamp(s), q_GtoI, p_IinG), use double here for timestamp
   */
   ...
   */
  bool get_pose(double timestamp, Mat3 &R_GtoI, Vec3 &p_IinG);
   ...
   * @return False if we can't find it
   */
  bool get_velocity(double timestamp, Mat3 &R_GtoI, Vec3 &p_IinG, Vec3 &w_IinI,
                    Vec3 &v_IinG);
   ...
   * @return False if we can't find it
   */
  bool get_acceleration(double timestamp, Mat3 &R_GtoI, Vec3 &p_IinG,
...
protected:
  ...
  typedef std::map<double, Mat4, std::less<double>,
                   Eigen::aligned_allocator<std::pair<const double, Mat4>>>
      AlignedEigenMat4d;
  ...
  static bool find_bounding_control_points(const double timestamp,
                                           ...
                                           double &t0, Mat4 &pose0, double &t1,
                                           Mat4 &pose1, double &t2, Mat4 &pose2,
