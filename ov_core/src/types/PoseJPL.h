...
public:
  PoseJPL() : Type(6) {
    ...
    Eigen::Matrix<DataType, 7, 1> pose0;
    pose0.setZero();
    pose0(3) = 1.0;
  ...
  void update(const VecX &dx) override {
    ...
    Eigen::Matrix<DataType, 7, 1> newX = _value;
  ...
  /// Rotation access
  Mat3 Rot() const { return _q->Rot(); }

  /// FEJ Rotation access
  Mat3 Rot_fej() const { return _q->Rot_fej(); }

  /// Rotation access as quaternion
  Vec4 quat() const { return _q->value(); }

  /// FEJ Rotation access as quaternion
  Vec4 quat_fej() const { return _q->fej(); }

  /// Position access
  Vec3 pos() const { return _p->value(); }

  // FEJ position access
  Vec3 pos_fej() const { return _p->fej(); }
