...
public:
  ...
  void update(const VecX &dx) override {
    ...
    Eigen::Matrix<DataType, 16, 1> newX = _value;
  ...
  /// Rotation access
  Mat3 Rot() const { return _pose->Rot(); }

  /// FEJ Rotation access
  Mat3 Rot_fej() const { return _pose->Rot_fej(); }

  /// Rotation access quaternion
  Vec4 quat() const { return _pose->quat(); }

  /// FEJ Rotation access quaternion
  Vec4 quat_fej() const { return _pose->quat_fej(); }

  /// Position access
  Vec3 pos() const { return _pose->pos(); }

  /// FEJ position access
  Vec3 pos_fej() const { return _pose->pos_fej(); }

  /// Velocity access
  Vec3 vel() const { return _v->value(); }

  // FEJ velocity access
  Vec3 vel_fej() const { return _v->fej(); }

  /// Gyro bias access
  Vec3 bias_g() const { return _bg->value(); }

  /// FEJ gyro bias access
  Vec3 bias_g_fej() const { return _bg->fej(); }

  /// Accel bias access
  Vec3 bias_a() const { return _ba->value(); }

  // FEJ accel bias access
  Vec3 bias_a_fej() const { return _ba->fej(); }
