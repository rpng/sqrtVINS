 ...
 *
 * This has the core algorithms that all projects within the OpenVINS ecosystem
 * leverage. The purpose is to allow for the reuse of code that could be shared
 * between different localization systems (i.e. msckf-based, batch-based, etc.).
 * These algorithms are the foundation which is necessary before we can even
 * write an estimator that can perform localization. The key components of the
 ...
 *
 * Please take a look at classes that we offer for the user to leverage as each
 * has its own documentation. If you are looking for the estimator please take a
 * look at the ov_msckf project which leverages these algorithms. If you are
 * looking for the different types please take a look at the ov_type namespace
 ...
 * @endcode
 *
 * When deriving Jacobians, it is important to ensure that the error state used
 * matches the type. Each type updates an update function, and thus directly
 * defines the error state it has. A type with non-trivial error states is the
 * @ref ov_type::JPLQuat which has equivalent quaterion and SO(3) error. For
 * rotations and on-manifold representations, [State Estimation for
 * Robotics](http://asrl.utias.utoronto.ca/~tdb/bib/barfoot_ser17.pdf) by
 * Timothy D. Barfoot @cite Barfoot2017 covers a nice range of examples.
 *
 */
namespace ov_type {}
