...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  if (mask_ll.empty()) {
    ...
    PRINT_ERROR(RED "[KLT-EXTRACTOR]: Failed to get enough points to do "
                    "RANSAC, resetting.....\n" RESET);
    return;
  }
  ...
  for (size_t i = 0; i < good_left.size(); i++) {
    cv::Point2f npt_l = camera_calib.at(cam_id)->undistort(good_left.at(i).pt);
...
void TrackKLT::feed_stereo(const CameraData &message, size_t msg_id_left,
  ...
  for (size_t i = 0; i < good_left.size(); i++) {
    cv::Point2f npt_l =
        camera_calib.at(cam_id_left)->undistort(good_left.at(i).pt);
  ...
  for (size_t i = 0; i < good_right.size(); i++) {
    cv::Point2f npt_r =
        camera_calib.at(cam_id_right)->undistort(good_right.at(i).pt);
...
void TrackKLT::perform_detection_monocular(const std::vector<cv::Mat> &img0pyr,
  ...
  DataType min_feat_percent = 0.50;
...
void TrackKLT::perform_matching(const std::vector<cv::Mat> &img0pyr,
  ...
  for (size_t i = 0; i < pts0.size(); i++) {
    pts0_n.push_back(camera_calib.at(id0)->undistort(pts0.at(i)));
    pts1_n.push_back(camera_calib.at(id1)->undistort(pts1.at(i)));
  }

  // Do RANSAC outlier rejection (note since we normalized the max pixel error
  // is now in the normalized cords)
  std::vector<uchar> mask_rsc;
  DataType max_focallength_img0 = std::max(camera_calib.at(id0)->get_K()(0, 0),
                                           camera_calib.at(id0)->get_K()(1, 1));
  DataType max_focallength_img1 = std::max(camera_calib.at(id1)->get_K()(0, 0),
