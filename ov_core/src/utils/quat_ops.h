...
rot_2_quat(const Eigen::MatrixBase<Derived> &rot) {
...
skew_x(const Eigen::MatrixBase<Derived> &w) {
  Eigen::Matrix<typename Derived::Scalar, 3, 3> w_x;
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3>
quat_2_Rot(const Eigen::MatrixBase<Derived> &q) {
  Eigen::Matrix<typename Derived::Scalar, 3, 3> q_x =
      skew_x(q.template block<3, 1>(0, 0));
  Eigen::Matrix<typename Derived::Scalar, 3, 3> Rot =
      (2 * std::pow(q(3, 0), 2) - 1) *
          Eigen::Matrix<typename Derived::Scalar, 3, 3>::Identity() -
      2 * q(3, 0) * q_x +
      2 * q.template block<3, 1>(0, 0) *
          q.template block<3, 1>(0, 0).transpose();
  return Rot;
}
...
quat_multiply(const Eigen::MatrixBase<Derived1> &q,
              const Eigen::MatrixBase<Derived2> &p) {
  Eigen::Matrix<typename Derived1::Scalar, 4, 1> q_t;
  Eigen::Matrix<typename Derived1::Scalar, 4, 4> Qm;
  // create big L matrix
  Qm.block(0, 0, 3, 3) =
      q(3, 0) * Eigen::Matrix<typename Derived1::Scalar, 3, 3>::Identity() -
      skew_x(q.template block<3, 1>(0, 0));
  Qm.block(0, 3, 3, 1) = q.template block<3, 1>(0, 0);
  Qm.block(3, 0, 1, 3) = -q.template block<3, 1>(0, 0).transpose();
  Qm(3, 3) = q(3, 0);
  q_t = Qm * p;
  // ensure uniqueness by forcing q_4 to be > 0
  if (q_t(3, 0) < 0) {
    q_t *= -1;
  }
  // normalize and return
...
vee(const Eigen::MatrixBase<Derived> &w_x) {
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3>
exp_so3(const Eigen::MatrixBase<Derived> &w) {
  // get theta
  Eigen::Matrix<typename Derived::Scalar, 3, 3> w_x = skew_x(w);
  typename Derived::Scalar theta = w.norm();
  // Handle small angle values
  typename Derived::Scalar A, B;
  if (theta < 1e-7) {
    A = 1;
    B = 0.5;
  } else {
  ...
  if (theta == 0) {
    R = Eigen::Matrix<typename Derived::Scalar, Eigen::Dynamic,
                      Eigen::Dynamic>::Identity(3, 3);
  } else {
    R = Eigen::Matrix<typename Derived::Scalar, Eigen::Dynamic,
                      Eigen::Dynamic>::Identity(3, 3) +
        A * w_x + B * w_x * w_x;
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 1>
log_so3(const Eigen::MatrixBase<Derived> &R) {

  // note switch to base 1
  typename Derived::Scalar R11 = R(0, 0), R12 = R(0, 1), R13 = R(0, 2);
  typename Derived::Scalar R21 = R(1, 0), R22 = R(1, 1), R23 = R(1, 2);
  typename Derived::Scalar R31 = R(2, 0), R32 = R(2, 1), R33 = R(2, 2);

  // Get trace(R)
  const typename Derived::Scalar tr = R.trace();
  ...
  if (tr + 1.0 < 1e-10) {
    if (std::abs(R33 + 1.0) > 1e-5)
      omega =
          (M_PI / sqrt(2.0 + 2.0 * R33)) *
          Eigen::Matrix<typename Derived::Scalar, 3, 1>(R13, R23, 1.0 + R33);
    else if (std::abs(R22 + 1.0) > 1e-5)
      omega =
          (M_PI / sqrt(2.0 + 2.0 * R22)) *
          Eigen::Matrix<typename Derived::Scalar, 3, 1>(R12, 1.0 + R22, R32);
    else
      // if(std::abs(R.r1_.x()+1.0) > 1e-5)  This is implicit
      omega =
          (M_PI / sqrt(2.0 + 2.0 * R11)) *
          Eigen::Matrix<typename Derived::Scalar, 3, 1>(1.0 + R11, R21, R31);
  } else {
    ...
    omega = magnitude * Eigen::Matrix<typename Derived::Scalar, 3, 1>(
...
exp_se3(const Eigen::MatrixBase<Derived> &vec) {

  // Precompute our values
  Eigen::Matrix<typename Derived::Scalar, 3, 1> w = vec.head(3);
  Eigen::Matrix<typename Derived::Scalar, 3, 1> u = vec.tail(3);
  typename Derived::Scalar theta = sqrt(w.dot(w));
  Eigen::Matrix<typename Derived::Scalar, 3, 3> wskew = skew_x(w);
  // wskew << 0, -w(2), w(1), w(2), 0, -w(0), -w(1), w(0), 0;

  // Handle small angle values
  typename Derived::Scalar A, B, C;
  if (theta < 1e-7) {
    A = 1;
    B = 0.5;
    C = 1.0 / 6.0;
  ...
  Eigen::Matrix<typename Derived::Scalar, 3, 3> I_33 =
      Eigen::Matrix<typename Derived::Scalar, 3, 3>::Identity();
  Eigen::Matrix<typename Derived::Scalar, 3, 3> V =
      I_33 + B * wskew + C * wskew * wskew;
 ...
 */
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 6, 1>
log_se3(const Eigen::MatrixBase<Derived> &mat) {
  Eigen::Matrix<typename Derived::Scalar, 3, 1> w =
      log_so3(mat.template block<3, 3>(0, 0));
  Eigen::Matrix<typename Derived::Scalar, 3, 1> T =
      mat.template block<3, 1>(0, 3);
  const typename Derived::Scalar t = w.norm();
  if (t < 1e-10) {
    Eigen::Matrix<typename Derived::Scalar, 6, 1> log;
    log << w, T;
    return log;
  } else {
    Eigen::Matrix<typename Derived::Scalar, 3, 3> W = skew_x(w / t);
    // Formula from Agrawal06iros, equation (14)
    // simplified with Mathematica, and multiplying in T to avoid matrix math
    typename Derived::Scalar Tan = tan(0.5 * t);
    Eigen::Matrix<typename Derived::Scalar, 3, 1> WT = W * T;
    Eigen::Matrix<typename Derived::Scalar, 3, 1> u =
        T - (0.5 * t) * WT + (1 - t / (2. * Tan)) * (W * WT);
    Eigen::Matrix<typename Derived::Scalar, 6, 1> log;
    log << w, u;
    return log;
  }
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 4>
hat_se3(const Eigen::MatrixBase<Derived> &vec) {
  Eigen::Matrix<typename Derived::Scalar, 4, 4> mat =
      Eigen::Matrix<typename Derived::Scalar, 4, 4>::Zero();
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 4>
Inv_se3(const Eigen::MatrixBase<Derived> &T) {
  Eigen::Matrix<typename Derived::Scalar, 4, 4> Tinv =
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 1>
Inv(const Eigen::MatrixBase<Derived> &q) {
  Eigen::Matrix<typename Derived::Scalar, 4, 1> qinv;
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 4>
Omega(const Eigen::MatrixBase<Derived> &w) {
  Eigen::Matrix<typename Derived::Scalar, 4, 4> mat;
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 1>
quatnorm(const Eigen::MatrixBase<Derived> &q_t) {
  Eigen::Matrix<typename Derived::Scalar, 4, 1> result = q_t;
  if (result(3, 0) < 0) {
    result *= -1;
  }
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3>
Jl_so3(const Eigen::MatrixBase<Derived> &w) {
  typename Derived::Scalar theta = w.norm();
  if (theta < 1e-6) {
    return Eigen::Matrix<typename Derived::Scalar, Eigen::Dynamic,
                         Eigen::Dynamic>::Identity(3, 3);
  } else {
    Eigen::Matrix<typename Derived::Scalar, 3, 1> a = w / theta;
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 3>
Jr_so3(const Eigen::MatrixBase<Derived> &w) {
  return Jl_so3(-w);
}
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 1>
rot2rpy(const Eigen::MatrixBase<Derived> &rot) {
 ...
 * @param t roll angle
 */
template <typename Scalar> inline Eigen::Matrix<Scalar, 3, 3> rot_x(Scalar t) {
 ...
 * @param t pitch angle
 */
template <typename Scalar> inline Eigen::Matrix<Scalar, 3, 3> rot_y(Scalar t) {
  Eigen::Matrix<Scalar, 3, 3> r;
  Scalar ct = cos(t);
  Scalar st = sin(t);
 ...
 * @param t yaw angle
 */
template <typename Scalar> inline Eigen::Matrix<Scalar, 3, 3> rot_z(Scalar t) {
  Eigen::Matrix<Scalar, 3, 3> r;
  Scalar ct = cos(t);
  Scalar st = sin(t);
